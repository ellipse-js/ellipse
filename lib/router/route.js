...
Route.prototype = {
    ...
    dispatch: function dispatch(ctx, req, res, options, callback) {
        ...
        function next(err, code, message) {
            var done    = ctx.callback,
                handler = handlers[ i++ ],
                args    = arguments

            var p = app._promisify(function (resolve, reject) {
        ...
        function nextParam(err, code, message) {
            ...
            var p = app._promisify(function (resolve, reject) {
                ...
                // all params are processed, it's time to execute the handlers of this route
                    ...
                    try {
                        if (handler.handle)
                            handler.handle(ctx, req, res, nextParam, params[ param.index ])
                        else
                            handler(ctx, req, res, nextParam, params[ param.index ])
                    }
                    catch (ex) {
                        err = httpError(ex)
                        error(err, ctx, req, res)
