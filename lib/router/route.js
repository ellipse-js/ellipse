...
Route.prototype = {
    ...
    dispatch: function dispatch(ctx, req, res, options, callback) {
        const self     = this,
              ...
              onerror  = this.onerror.bind(this, ctx)

        // start chain
        return defer(dispatch).catch(onerror)
        ...
        function dispatch(i) {
            ...
            function next(err) {
                ...
                if (err)
                    ...
                    return dispatch(i + 1)//.catch(onerror)
            }
        }
    },
...
function defer(fn) {
    return new Promise(resolve => setImmediate(() => resolve(fn(0))))
}
