...
Router.prototype = {
    ...
    handle: function handle(ctx, req, res, next) {
        ...
        if(!ctx)
            ctx = new Context(req, res) // constructor sets `req._ctx`
    ...
    dispatch: function dispatch(ctx, req, res, next) {
        ...
        function nextRoute() {
            ...
            // except if automatic response has been configured
                ...
                if (ctx.respond || app.respond)
                    return ctx.send()

                router.emit('not found', ctx, req, res)

                if(!listenerCount(router, 'not found'))
                    handleMissing(ctx)
            }
        }
    },
    ...
    use: function use(path, handlers) {
        ...
        if(path !== '/*' && path !== '*') {
            ...
            this.emit('mount', path.substring(0, index), this)
        }
