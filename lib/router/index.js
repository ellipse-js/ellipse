...
var Emitter       = require('events').EventEmitter, // todo: `events` exposes EventEmitter in node >0.10
    ...
    isGenFunc     = require('../utils/checkGenerator').isGeneratorFunction,
...
Router.prototype = {
    ...
    handle: function handle(ctx, req, res, next) {
        ...
        if (!ctx)
            ctx = new Context(req, res) // note: constructor sets `req._ctx`
    ...
    dispatch: function dispatch(ctx, req, res, next) {
        ...
        function nextRoute() {
            ...
            // except if automatic response has been configured
                ...
                if (!router.listenerCount('not found'))
                    handleMissing(ctx)
            }
        }
    },
