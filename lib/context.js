...
prototype(Context, {
    ...
    _onPipe: function onPipe(stream) {
        ...
        // proxy events
        res.on('close', function onclose() { ctx.emit('close') })
        res.on('finish', function onfinish() { ctx.emit('finish') })
        res.on('drain', function ondrain() { ctx.emit('drain') })
        res.on('error', function onerror(err) { ctx.emit('error', err) })
        res.on('unpipe', function onunpipe(stream) { ctx.emit('unpipe', stream) })
