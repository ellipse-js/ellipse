...
Object.defineProperties(Router.prototype, {
    ...
    init: {
        value: function init() {
            ...
            this.route('*', '*', function init(req, res, next) {
                var app = self.app
                ...
                // initialize cookies
                    ...
                    Cookies.express(app.keys)(req, res, next)
            })
        }
    },
    ...
    handle: {
        value: function handle(ctx, req, res, fn, index) {
            ...
            if(!ctx) {
                ctx = req.ctx || new Context(req, res)
                merge(ctx, this.app.context)
            }
    ...
    processParams: {
        value: function processParams(route, match, params, ctx, req, res, fn, index) {
            var self = this,
                i    = 0
    ...
    route: {
        value: function route(method, path, handlers) {
            ...
            for(i = 0, l = handlers.length; i < l; i++) {
                ...
                assert(handler instanceof Function || handler instanceof Router, 'Request handler must be a function or a Router instance.')
