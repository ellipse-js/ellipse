...
Object.defineProperties(Router.prototype, {
                ...
                if(
                    ...
                    if(options) {
                        ...
                        if(route.params.length)
                            this.processParams(route, match, params, req, res, fn)
    ...
    processParams: {
        value: function (route, match, params, req, res, fn, i) {
            var self = this

            if (i === undefined)
                i = 0

            nextParam(route, match, params, 0)

            function nextParam(route, match, params, i) {
                if (i >= route.params.length)
                    return self.chain(route, params, req, res, fn, i)

                var param = route.params[ i ]

                if (param in self.params)
                    self.params[ param ](req, res, function (err, code, message) {
                        if (err)
                            return error(err, req, res, code, message)
                        else
                            nextParam(route, match, params, i + 1)
                    }, params[ param ])
                else
                    nextParam(route, match, params, i + 1)
            }
    ...
    chain: {
        value: function chain(route, params, req, res, fn, i) {
            var self     = this,
