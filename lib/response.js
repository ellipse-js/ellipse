...
function redirect(status, url, alt) {
    ...
    if (this.req.accepts('text/html')) {
        url = escape(url)
        this.type('text/html')
        this.body = `<p>${statusCodes[ status ]}. Redirecting to <a href="${url}">${url}</a>.</p>`
    }
    else {
        this.type('text/plain')
        this.body = `${statusCodes[ status ]}. Redirecting to ${url}.`
    }
 ...
 */
  ...
  */
  function jsonp() {
      throw new Error(
          'res.jsonp() is not implemented. Ellipse does not support JSONP.'
