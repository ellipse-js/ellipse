...
function respond(body) {
    ...
    if (body && req.method !== 'HEAD') {
        if (kind === 'stream') {
            ...
            body.on('close', () => destroy(body))

            // handle stream errors if nobody else does
            if (!body.listenerCount('error')) {
                // todo: generator mws? maybe add a `catch()` fn to req/res/ctx?
                var next = this.next
...
function setResponseHeader(field, value) {
    if (typeof field === 'object')
        Object.keys(field).forEach(k =>
            this.setHeader(k.toLowerCase(), field[ k ]))
    else
...
function removeResponseHeader(field) {
    if (Array.isArray(field))
        field.forEach(f => this.removeHeader(f.toLowerCase()))
    else
...
function sendFile(path, options, callback) {
    // todo: it sould return a promise
    ...
    sendfile(res, file, opts, err => {
        ...
        if (err && err.code === 'EISDIR')
            return next() // todo: next is not always a function

        // next() all but write errors
        if (err && err.code !== 'ECONNABORTED' && err.syscall !== 'write')
            next(err) // todo: next is not always a function
    })
}
